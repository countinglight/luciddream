...
v2 runs on the existing React Native / TypeScript stack, hardened. The owner's answers to the open
questions and the follow-up answers (same day) are integrated: decisions D14-D29 in §8, the status
of every question in §9. Only the store-rules review (§9.2) remains open.
...
- **The host is a service that happens to run on the user's own machine.** Local server, browser
  ...
  not a rewrite. v2 starts strictly with the desktop host; the cloud backend decision is postponed
  (D25).
...
- **Provider behind an adapter, cost visible.** The user chooses a local model server or a paid cloud
  provider (lower-end models by default, D15) and sees what is sent.
...
| F5.6 | **Night-length period presets (must).** v1 ships `$short`/`$medium`/`$long` as 5 s / 20 s / 5 min, a usability-test cadence chosen for the v1 beta and not night values. v2 must replace them with real night defaults, defined once in `src/engine/duration.ts` as today (D30).                                                                                                                                                                             | G5, G6 | S1, S9    |
...
| Id   | Feature                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Goals | Scenarios |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | --------- |
| F9.1 | **Store distribution, both platforms, at a v2.x** after the night-audio story has been through beta: Play and App Store listings, privacy and health-data declarations, review readiness for background audio (see F2.4). Not a v1 milestone (D36). Known prerequisites: a Google Play developer account ($25 one-time), opened as an **organization** (Counting Light, LLC), because new personal accounts must first run a closed test with 12 testers for 14 days; the Play Data safety form and the media-playback foreground-service declaration; store screenshots; App Store review notes explaining overnight background audio; a privacy-policy URL (the website's `/privacy/`). Already in place from v1: the App Store Connect app record, its App Privacy answers, TestFlight groups, EAS signing for both platforms. | G6    | S10       |
...
| F9.3 | **Beta channels stay**: APK on GitHub Releases and TestFlight for v2.0 and v2.1. Includes the **external TestFlight testing** v1 deliberately did without (v1 used an Internal group only, decided 2026-09-17): a public invitation link, the Beta App Review it requires, tester onboarding on the website, and no App Store Connect account per tester.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | G6    | S10       |
...
| F9.6 | **Owner access to beta diagnostics.** Tools and an interface for the owner to download telemetry from the ingest service, protected by authentication. The v1 Worker deliberately has no read endpoint; queries today go through Wrangler's authenticated API (`npm run telemetry:nights`). Whatever replaces that must never be reachable with the app's public ingest token (D35).                                                                                                                                                                                                                                                                                                                                                                                                                                              | G5    | S10       |
...
added, not a rewrite. v2 starts strictly with this desktop host; the decision on a real cloud
backend is postponed (D25). Until then, the host has
...
bundle arrives (shared from the phone into a watched folder, or uploaded through the page). They drop the recording file next to it. The host finds LucidDream's cues in the
...
as a file to share to the phone. Declining records why.
...
|                   -> local model server | paid cloud provider    |
...
- Audio decoding of recorder formats needs a decoder on the host; prefer a pure-JavaScript or
  ...
  exotic formats.
- Two provider kinds through the adapter, both first-class (D15): a **local model server** reached
  over an OpenAI-compatible endpoint (the owner's reference machine is a DGX Spark on the home
  network; any capable machine works), and a **paid cloud provider** using lower-end models by
  default. The host machine itself needs no GPU. The setting is visible and per-profile. When the user
  runs the host themselves, cloud usage is billed to their own API key.
...
The decision is postponed (D25). The owner's considerations, in both directions:

- **Against moving early.** A hosted service brings paid subscriptions, and running billing, support
  and quotas for a growing user count is operational work that would slow the product. Building and
  operating a backend distracts from the v2 quality bar (D14).
- **For moving.** Users unwilling to install and run a local tool could make that a big issue for
  adoption: the host is where v2's value is, and a local tool puts it out of reach for some.
...
Beginners are not the v2 audience; a single-device path waits for v3 evidence that it is wanted. If
users ask for a LucidDream recorder, it can move into v2 (F7.3, D21).
...
- **v2.0:** eight-hour physical-device runs pass on both platforms (F2.7); softening presets have
  unit fixtures and a listening sign-off from the customer who raised #6, on a bedside speaker; the
  sound check (F1.10) works end to end; a bundle exported from the
...
release automation from [luciddream-ios-support-plan.md](../archive/luciddream-ios-support-plan.md).

### 7.4 Carried over from the v1 hardening pass (2026-09-15/16)

Moved here from the hardening summary
([v1-hardening-091526.md](../dev_process/v1-hardening-091526.md) §11), which is being archived.
Minor inconsistencies from the same pass are tracked in
[issue #7](https://github.com/countinglight/luciddream/issues/7). They are **not** v2 carry-over:
on 2026-09-17 the owner made them v1 release blockers (see
[v1-finish-plan-091626.md](../dev_process/v1-finish-plan-091626.md) §4, W1-W4).

**Deferred features and fixes**

- **Run manifest** (architectural review AR-10 / A8): a first log record pinning app build,
  platform, time zone, settings and each phase's script text, so a shared log explains itself. The
  natural next piece of the night record, and needed before F4.3's bundle.
- **Checkpoint and resume** (F2.1), **Android exact alarms** (F2.3), **cue-time notifications**
  (F2.5), **night ambience** (F2.4) and **battery measurement** (F2.8, which needs `expo-battery`, a
  new native dependency). All placed in v2 by both architectural reviews.
- **True/false/unknown condition semantics** (F3.1). Must not reach v1: it changes which cues an
  existing script plays.
- **Clock conditions across midnight** (AR-23): `until: { clock: { gte: "06:00" } }` is already true
  at 23:00, so such a loop ends at bedtime. Documented in the v1 spec §3.3, not changed.
- **Download size and time limits** for Library imports (A4). Downloads are staged, so a partial one
  is discarded, but nothing bounds a very large or very slow one.
- **More session records** (AR-11): voice-interrupt ducking and resume, a write-failure count in the
  stop record, per-run sequence numbers. They go in `SessionRecord` (`src/logging/records.ts`);
  `NightSession.note()` already writes one kind.
- **Night-length period presets** — F5.6, a must.
- **Owner access to diagnostics** — F9.6.

**Test coverage known to be missing**

- `ExpoFileSystemStore` and `WebFileStore` are not run against the `FileStorePort` contract test;
  only the in-memory fake is. The native store needs a device; the web store needs a real IndexedDB
  (`fake-indexeddb` would be a new devDependency).
- `src/session/session.ts`, the real Expo wiring, has no test. The lifecycle above it is tested
  through `NightSession` with injected fakes.

### 7.5 What would pull v3 work forward
...
### 8.3 Decisions from the v1 hardening review (2026-09-16)

| #   | Decision                                                                                                                                                                                                                                                           |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D30 | Period presets stay at 5 s / 20 s / 5 min for the v1 beta, which is what the usability test asked for. v2 must replace them with night-length defaults (F5.6).                                                                                                     |
| D31 | On Android, a night takes exclusive audio focus, because that is what keeps playback alive with the screen off; the Audio focus setting is not applied there. Accepted for v1 and stated in the release notes: the effect is limited to the moment a night starts. |
| D32 | Scripts are validated when added to the Library, and unknown keys are rejected by name. Bundled scripts are trusted.                                                                                                                                               |
| D33 | Library addresses are HTTPS only. Testers with a saved `http://` address re-add it.                                                                                                                                                                                |
| D34 | The run log records every event; logging categories filter display and export only. Log size is revisited only if users report it.                                                                                                                                 |
| D35 | Downloading diagnostics is an owner tool behind authentication (F9.6), separate from the public ingest endpoint.                                                                                                                                                   |

### 8.4 Decisions from the v1 finish review (2026-09-16)

| #   | Decision                                                                                                                                                                                |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D36 | Publishing to Google Play and the App Store moves to v2 (F9.1). v1 ends with beta distribution only: APK on GitHub Releases and TestFlight. No store milestone is added to the v1 spec. |
...
**4. Store review of background audio.** This question does not depend on a cloud backend: it
concerns only the phone app in the App Store and Google Play, and applies even with no service at
...
alarm module is built, because its design depends on the result. Points to settle:

- whether LucidDream can use Google's alarm-app exact-alarm permission, or must ask the user to grant
  "Alarms & reminders". In the second case, F2.3 includes a first-run prompt that walks the user to
  the system setting, and the morning report (F2.9) says when it was not granted;
- what the Play foreground-service declaration for media playback requires (text, video);
- whether Apple's background-audio rule accepts the night ambience (F2.4) as described, including
  when the user turns it down or off;
- what the Health Connect and HealthKit declarations require for the data chosen in D26.

### 9.3 Alternatives kept for the usability test

Background on wearables (D24, D26, D27), kept for implementation: an Apple Watch only reaches an
iPhone's HealthKit. Samsung Galaxy Watch data reaches Health Connect through Samsung Health, where the
user must enable sync, and Google Pixel Watch data through the Fitbit app. Oura writes to Health
Connect on Android and to HealthKit on iOS. Vendor apps often write last night's sleep only once the
user opens them in the morning, and the fields each device fills in need verifying on hardware.

**Visual editor (D28).** The chosen shape, a step list with one level of blocks, works like this:
`repeat`, `if`/`else` and `with` are block cards that open an indented list one level deep. Inside a
`repeat` block, the "add step" menu hides `repeat`, so a nested loop cannot be created. Conditions use
a picker (field, comparison, value) with an all/any switch. It stays closest to the YAML, so a round
trip loses nothing. Alternatives to revisit if the usability test goes badly:

- **Phase-structured form.** The three phases of the Dream Console each hold a step list, and
  blocks live inside phases. Friendliest for the common night shape, but scripts that don't follow
  phases fall back to A anyway, so there are two editors to maintain.
- **Time-axis editor.** Steps laid out along the night's clock with drag to move. Intuitive for
  timing, but it cannot show `if` branches or `until` loops honestly, because their duration is only
  known at run time. Better as a read-only view (F5.5) than as an editor.

The chosen handling of scripts the editor cannot represent is to open them read-only with "Edit as
text". Alternatives:

- **Nested loop as a locked block.** Everything around it is editable; the inner loop shows as one
  sealed card editable only as text. More capable, more UI to build.
- **Offer to flatten.** A finite inner `repeat: 3` is expanded into three copies of its body, after
  which the script is fully editable. Not possible for `infinite` or `until` loops, so only a partial
  answer.

**On-device sensing (D29), starting point for measurement:** accelerometer summarised into movement
per 30-second window from low-rate sampling, sound level once a second from the existing metering.
Engineering escalates to the owner only if the 8 % per-night budget (§1.4) cannot be met.
