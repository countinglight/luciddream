...
describe("ensureResolved (url source)", () => {
  it("downloads to cache on first resolution", async () => {
    ...
    const result = await ensureResolved(
      "signals",
      "sig1",
      { type: "url", url: "https://x.test/a.mp3" },
      store,
      "audio",
    );

    expect(result).toEqual({ root: "cache", path: "signals/sig1.mp3" });
    // Staged, then promoted: the download itself never writes to the path
    // resolution will later trust.
    expect(store.downloads).toEqual([
      {
        url: "https://x.test/a.mp3",
        root: "cache",
        path: "signals/sig1.mp3.part",
      },
    ]);
    expect(await store.exists("cache", "signals/sig1.mp3")).toBe(true);
    expect(await store.exists("cache", "signals/sig1.mp3.part")).toBe(false);
  });
  ...
  it("falls back to the default extension when the URL has none", async () => {
    ...
    const result = await ensureResolved(
      "scripts",
      "scr1",
      { type: "url", url: "https://x.test/script" },
      store,
      "yaml",
    );
...
describe("ensureResolved (file source)", () => {
  it("resolves to the stored path without downloading anything", async () => {
    const store = new InMemoryFileStore();
    await store.writeText("document", "signals/sig1.mp3", "audio");

    const result = await ensureResolved(
      "signals",
      "sig1",
      { type: "file", path: "signals/sig1.mp3" },
      store,
      "audio",
    );
  ...
  it("refuses an imported file that is no longer on the phone", async () => {
    // The path used to be trusted without checking, so a copy lost to a
    // restore or an OS clean-up passed preflight and failed hours later as an
    // unplayable cue in the dark.
    const store = new InMemoryFileStore();
    ...
    await expect(
      ensureResolved(
        "signals",
        "sig1",
        { type: "file", path: "signals/gone.mp3" },
        store,
        "audio",
      ),
    ).rejects.toThrow(/no longer on the phone/);
  });
  ...
  it("leaves nothing behind when a download fails part way", async () => {
    ...
    expect(await store.exists("cache", "signals/sig1.mp3")).toBe(false);
    expect(await store.exists("cache", "signals/sig1.mp3.part")).toBe(false);
  });
});
...
describe("saveOffline", () => {
  ...
  it("downloads straight to document storage when nothing was cached yet", async () => {
    ...
    expect(await store.exists("document", "signals/sig1.mp3")).toBe(true);
    expect(store.downloads).toEqual([
      { url: source.url, root: "document", path: "signals/sig1.mp3" },
    ]);
  });
...
describe("removeOfflineCopy / isSavedOffline", () => {
  it("deletes only the document copy, leaving the cache alone", async () => {
    ...
    expect(
      await isSavedOffline("signals", "sig1", source, store, "audio"),
    ).toBe(false);
    expect(await store.exists("cache", "signals/sig1.mp3")).toBe(true);
  });
