...
describe("runScript", () => {
  ...
  it("does not await a fire-and-forget play, but does await wait: true", async () => {
    ...
    expect(deps.log.events.map((e) => e.type)).toEqual([
      "run.start",
      "play",
      "log",
      "run.stop",
  ...
  it("advances through wait statements only when the virtual clock is advanced", async () => {
    ...
    expect(
      deps.log.events.find((e) => e.type === "play" && e.signal === "bell"),
    ).toMatchObject({ at: 10_000 });
  });
...
`);
    ...
    expect(deps.audio.calls).toHaveLength(3);
    expect(deps.log.events.at(-1)).toMatchObject({
      type: "run.stop",
      reason: "completed",
    });
  });
...
`);
    ...
    expect(deps.audio.calls.map((c) => c.signal)).toEqual(["chime", "bell"]);
    expect(deps.log.events).toContainEqual({
  ...
  it("reports a runtime error as an error event and ends the run instead of throwing", async () => {
    ...
    expect(deps.log.events).toContainEqual({
      type: "error",
      at: 0,
      message: "audio backend exploded",
    });
    expect(deps.log.events.at(-1)).toEqual({
      type: "run.stop",
      at: 0,
