...
describe("evaluateCondition", () => {
  it("evaluates a field condition with an explicit comparator", () => {
    ...
    expect(evaluateCondition(cond, { hr: 55 })).toEqual({
      value: true,
      missingFields: [],
    });
    expect(evaluateCondition(cond, { hr: 65 })).toEqual({
      value: false,
      missingFields: [],
    });
  });
  ...
  it("supports all six comparators", () => {
    const make = (comparator: ComparatorOp): Condition => ({
      kind: "field",
      field: "hr",
      comparator,
      value: 60,
  ...
  it("treats a missing field as false and reports it as missing", () => {
    ...
    expect(evaluateCondition(cond, {})).toEqual({
      value: false,
      missingFields: ["hrv"],
    });
  });
  ...
  it("compares clock strings and elapsed durations as already-normalized values", () => {
    ...
    const elapsed: Condition = {
      kind: "field",
      field: "elapsed",
      comparator: "gte",
      value: 8 * 3_600_000,
    };
    expect(evaluateCondition(elapsed, { elapsed: 8 * 3_600_000 }).value).toBe(
      true,
    );
