...
describe('settings persistence', () => {
  ...
  it('fills in defaults for fields missing from an older saved shape', async () => {
    await AsyncStorage.setItem('luciddream.settings.v1', JSON.stringify({ masterDefaultVolume: 0.9 }));

    expect(await loadSettings()).toEqual({ ...DEFAULT_SETTINGS, masterDefaultVolume: 0.9 });
  });
