...
describe("resolveSignal", () => {
  ...
  it("throws a readable error for an unknown bundled asset id", async () => {
    ...
    await expect(resolveSignal(item, new InMemoryFileStore())).rejects.toThrow(
      /Unknown bundled signal/,
    );
  });
...
body:
  ...
  - if: { rem: true }
    then:
      ...
      - play: e
`);
    expect(collectSignalNames(script)).toEqual(
      new Set(["a", "b", "c", "d", "e"]),
    );
  });
...
describe("resolveSignalMap", () => {
  ...
  it("throws naming the missing moniker when the library has no match", async () => {
    const store = new InMemoryFileStore();
    await expect(
      resolveSignalMap(["ghost"], [bundledChime], store),
